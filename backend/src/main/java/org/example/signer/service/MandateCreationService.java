...
import org.example.signer.xml.MandateCreationXmlGenerator;
...
public class MandateCreationService {
    ...
    public Map<String, String> executeMandateCreation(MandateCreationRequestDto requestDto) throws Exception {

        String newMsgId = generateMsgId(requestDto.getSourceId());
        String mandateId = generateMandateId();

        // Generate JAXB object
        MandateCreation mandate = MandateCreationXmlGenerator.generate(requestDto, newMsgId, mandateId);

        // Marshal JAXB object to Document
        Document doc = XmlUtils.marshalToDocument(mandate);
        XmlUtils.writeDocumentToFile(doc, "mandate_creation_generated.xml");
        ...
        XmlUtils.writeDocumentToFile(doc, "mandate_creation_signed.xml");

        // Encrypt the sensitive data in the signed document
        Encrypter.encrypt(doc, publicKey, "MndtInitnReq");
        XmlUtils.writeDocumentToFile(doc, "mandate_creation_encrypted.xml");
