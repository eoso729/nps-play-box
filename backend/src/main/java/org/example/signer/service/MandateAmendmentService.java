...
import org.example.signer.xml.MandateAmendmentXmlGenerator;
...
@Service
public class MandateAmendmentService {
    ...
    public Map<String, String> executeMandateAmendment(MandateAmendmentRequestDto requestDto) throws Exception {

        String newMsgId = generateMsgId(requestDto.getSourceId());

        // Generate JAXB object
        MandateAmendment mandate = MandateAmendmentXmlGenerator.generate(requestDto, newMsgId);

        // Marshal JAXB object to Document
        Document doc = XmlUtils.marshalToDocument(mandate);
        XmlUtils.writeDocumentToFile(doc, "mandate_amendment_generated.xml");
        ...
        XmlUtils.writeDocumentToFile(doc, "mandate_amendment_signed.xml");

        // Encrypt the sensitive data in the signed document
        Encrypter.encrypt(doc, publicKey, "MndtAmdmntReq");
        XmlUtils.writeDocumentToFile(doc, "mandate_amendment_encrypted.xml");
