...
import org.example.signer.xml.MandateCancellationXmlGenerator;
...
public class MandateCancellationService {
    ...
    public Map<String, String> executeMandateCancellation(MandateCancellationRequestDto requestDto) throws Exception {

        String newMsgId = generateMsgId(requestDto.getSourceId());

        // Generate JAXB object
        MandateCancellation mandateCancellation = MandateCancellationXmlGenerator.generate(requestDto, newMsgId);

        // Marshal JAXB object to Document
        Document doc = XmlUtils.marshalToDocument(mandateCancellation);
        XmlUtils.writeDocumentToFile(doc, "mandate_cancellation_generated.xml");
        ...
        XmlUtils.writeDocumentToFile(doc, "mandate_cancellation_signed.xml");

        // Encrypt the sensitive data in the signed document
        Encrypter.encrypt(doc, publicKey, "MndtCxlReq");
        XmlUtils.writeDocumentToFile(doc, "mandate_cancellation_encrypted.xml");

        String xmlContentToSend = XmlUtils.documentToString(doc);

        // Send the signed XML document
        String url = "http://10.8.8.132:8022/nps/pain";
